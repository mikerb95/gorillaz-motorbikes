...
router.get('/flujo-caja', requireAuth, requireAdmin, async (req, res) => {
  ...
  const months = Array.from({ length: 12 }, (_, i) => {
    ...
    const inv  = paidInvoices.filter(x => inPeriod(invIncomeDate(x), year, m)).reduce((s, x) => s + invIncome(x), 0);
...
router.get('/estado-resultados', requireAuth, requireAdmin, async (req, res) => {
  ...
  const prevIng  = paidInvoices.filter(i => inPeriod(invIncomeDate(i), prevYear, prevMonth)).reduce((s, i) => s + invIncome(i), 0) + filterPrev(paidOrders, 'createdAt').reduce((s, o) => s + o.total, 0);
