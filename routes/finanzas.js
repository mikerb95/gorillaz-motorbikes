...
router.get('/', requireAuth, requireAdmin, async (req, res) => {
  ...
  const totalIngresosAllTime = paidInvoices.reduce((s, i) => s + i.total, 0) + paidOrders.reduce((s, o) => s + o.total, 0);
  ...
  paidInvoices.filter(i => inPeriod(i.createdAt, year, month)).forEach(i => {
    byMethod[i.paymentMethod || 'efectivo'] = (byMethod[i.paymentMethod || 'efectivo'] || 0) + i.total;
  });
...
router.get('/estado-resultados', requireAuth, requireAdmin, async (req, res) => {
  ...
  const prevIng  = filterPrev(paidInvoices, 'createdAt').reduce((s, i) => s + i.total, 0) + filterPrev(paidOrders, 'createdAt').reduce((s, o) => s + o.total, 0);
