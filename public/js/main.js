...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Parse demand map injected by server
  let demandMap = {};
  try { demandMap = JSON.parse(calHost.dataset.demand || '{}'); } catch (e) { }
  ...
  const render = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (dateStr === fmt(today)) el.classList.add('is-today');
      const isPast = date < minDate;
      if (isPast) { el.classList.add('is-disabled'); el.disabled = true; }
      // Demand indicator
      const demandLevel = demandMap[dateStr];
      if (!isPast && demandLevel) {
        el.classList.add('demand-' + demandLevel);
        const dot = document.createElement('span');
        dot.className = 'cal-demand-dot';
        el.appendChild(dot);
        const labels = { low: 'Disponibilidad alta', medium: 'Alta demanda', high: 'Muy ocupado' };
        el.title = labels[demandLevel] || '';
      }
      if (input.value && dateStr === input.value) el.classList.add('is-selected');
